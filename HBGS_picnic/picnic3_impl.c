...
// Factor of 2 here because we have two LowMC calculations
#define MAX_AUX_BYTES_TEST5 \
    ((2 * (LOWMC_MAX_AND_GATES + LOWMC_MAX_KEY_BITS)) / 8 + 1)
...
static void createRandomTapes(randomTape_t *tapes,
  uint8_t **seeds,
  uint8_t *salt,
  size_t t,
  paramset_t *params)
...
#ifdef DEBUG_LMC
    printf("Orig: tape size given as: %ld\n", tapeSizeBytes);
#endif
 ...
 * input.
 */
static uint16_t extend(uint8_t bit) { return ~(bit - 1); }
  ...
  paramset_t *params)
{
    ...
    and_helper =
      parity16(and_helper) ^ getBit(tapes->tape[lastParty], tapes->pos - 1);
 ...
 * Updates the random tapes of all players with the mask values for the output
 * of AND gates, and computes the N-th party's share such that the AND gate
 * invariant holds on the mask values.
 */
static void computeAuxTape(
  randomTape_t *tapes, uint8_t *inputs, paramset_t *params)
{
...
#ifdef DEBUG_LMC
    printf("Tape position in ComputeAuxTape after key0, pos: %d\n", tapes->pos);
#endif

    // key = key0 x KMatrix[0]^(-1)
    matrix_mul(key, key0, KMatrixInv(0, params), params);

    if (inputs != NULL) { memcpy(inputs, key, params->stateSizeBytes); }
    ...
    for (uint32_t r = params->numRounds; r > 0; r--) {
        ...
        // y is the output from the S-box (calculated from the input to the next
        // S-box)
        if (r == 1) {
            // Use key as input
        ...
        tapes->pos =
          params->stateSizeBits * 2 * (r - 1) + params->stateSizeBits;
#ifdef DEBUG_LMC
        printf(
          "Tape position for ComputeAuxTape before aux_mpc_sbox %d, pos: %d\n",
          r,
          tapes->pos);
#endif
...
#ifdef DEBUG_LMC
        printf(
          "Tape position for ComputeAuxTape after aux_mpc_sbox %d, pos: %d\n",
          r,
          tapes->pos);
#endif
    }
...
static void commit(uint8_t *digest,
  uint8_t *seed,
  uint8_t *aux,
  uint8_t *salt,
  size_t t,
  ...
  paramset_t *params)
{
    ...
    if (aux != NULL) { HashUpdate(&ctx, aux, params->andSizeBytes); }
    HashUpdate(&ctx, salt, params->saltSizeBytes);
    HashUpdateIntLE(&ctx, t);
...
void commit_v(uint8_t *digest, uint8_t *input, msgs_t *msgs, paramset_t *params)
{
    HashInstance ctx;
...
{
    uint16_t and_helper =
      tapesToWord(tapes);// The special mask value setup during preprocessing
                         // for each AND gate
    uint16_t s_shares =
      (extend(a) & mask_b) ^ (extend(b) & mask_a) ^ and_helper;
    if (msgs->unopened >= 0) {
        uint8_t unopenedPartyBit =
          getBit(msgs->msgs[msgs->unopened], msgs->pos);
...
#ifdef DEBUG_VFY2
void print_simulateOnline_inputs(const uint32_t *maskedKey,
  const randomTape_t *tapes,
  const shares_t *tmp_shares,
  const msgs_t *msgs,
  const uint32_t *plaintext,
  const uint32_t *pubKey,
  const paramset_t *params)// nullptr for original picnic version
{
    printHex("maskedKey", (uint8_t *)maskedKey, params->stateSizeBytes);

    printf("There are %ld tapes, with pos=%d\n", tapes->nTapes, tapes->pos);
    size_t tape_size = getTapeSizeBytes(params);
    for (size_t t = 0; t < tapes->nTapes; ++t) {
        printf("t: %ld ", t);
        printHex("tape", tapes->tape[t], tape_size);
    }
    (void)tmp_shares;
    size_t msgSize = params->andSizeBytes;
    printf(
      "The messages have pos=%ld and unopened=%d\n", msgs->pos, msgs->unopened);
    for (size_t p = 0; p < params->numMPCParties; p++) {
        printf("p: %ld ", p);
        printHex("msg", msgs->msgs[p], msgSize);
    }
    printHex("plaintext", (uint8_t *)plaintext, params->stateSizeBytes);
    printHex("pubKey", (uint8_t *)pubKey, params->stateSizeBytes);
}
#endif

static int simulateOnline(uint32_t *maskedKey,
  randomTape_t *tapes,
  shares_t *tmp_shares,
  msgs_t *msgs,
  const uint32_t *plaintext,
  ...
  paramset_t *params)
{
    ...
    for (uint32_t r = 1; r <= params->numRounds; r++) {
        ...
        xor_array(
          state, roundKey, state, params->stateSizeWords);// state += roundKey
    }
...
static size_t bitsToChunks(
  size_t chunkLenBits, const uint8_t *input, size_t inputLen, uint16_t *chunks)
{
...
static size_t appendUnique(uint16_t *list, uint16_t value, size_t position)
{
    ...
    for (size_t i = 0; i < position; i++) {
        if (list[i] == value) { return position; }
    }
    list[position] = value;
    return position + 1;
}
  ...
  paramset_t *params)
{
    ...
    uint16_t *chunks =
      calloc(params->digestSizeBytes * 8 / MIN(bitsPerChunkC, bitsPerChunkP),
        sizeof(uint16_t));
    ...
    while (countC < params->numOpenedRounds) {
        size_t numChunks =
          bitsToChunks(bitsPerChunkC, h, params->digestSizeBytes, chunks);
        for (size_t i = 0; i < numChunks; i++) {
            if (chunks[i] < params->numMPCRounds) {
                countC = appendUnique(challengeC, chunks[i], countC);
            }
            if (countC == params->numOpenedRounds) { break; }
        }
    ...
    while (countP < params->numOpenedRounds) {
        size_t numChunks =
          bitsToChunks(bitsPerChunkP, h, params->digestSizeBytes, chunks);
        for (size_t i = 0; i < numChunks; i++) {
            if (chunks[i] < params->numMPCParties) {
                challengeP[countP] = chunks[i];
                countP++;
            }
            if (countP == params->numOpenedRounds) { break; }
        }
...
int verify_picnic3(signature2_t *sig,
  const uint32_t *pubKey,
  const uint32_t *plaintext,
  const uint8_t *message,
  size_t messageByteLength,
  paramset_t *params)
{
    ...
    tree_t *iSeedsTree =
      createTree(params->numMPCRounds, params->seedSizeBytes);

    int ret = reconstructSeeds(iSeedsTree,
      sig->challengeC,
      params->numOpenedRounds,
      sig->iSeedInfo,
      sig->iSeedInfoLen,
      sig->salt,
      0,
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        if (!contains(sig->challengeC, params->numOpenedRounds, t)) {
            ...
            seeds[t] = generateSeeds(params->numMPCParties,
              getLeaf(iSeedsTree, t),
              sig->salt,
              t,
              params);
        } else {
            ...
            size_t P_index =
              indexOf(sig->challengeC, params->numOpenedRounds, t);
            uint16_t hideList[1];
            hideList[0] = sig->challengeP[P_index];
            ret = reconstructSeeds(seeds[t],
              hideList,
              1,
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        ...
        if (!contains(sig->challengeC, params->numOpenedRounds, t)) {
            /* We're given iSeed, have expanded the seeds, compute aux from
             * scratch so we can comnpte Com[t] */
            ...
            size_t unopened = sig->challengeP[indexOf(
              sig->challengeC, params->numOpenedRounds, t)];
            for (size_t j = 0; j < last; j++) {
                if (j != unopened) {
                    commit(C[t].hashes[j],
                      getLeaf(seeds[t], j),
            ...
            memcpy(
              C[t].hashes[unopened], sig->proofs[t].C, params->digestSizeBytes);
        }
    }
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        if (contains(sig->challengeC, params->numOpenedRounds, t)) {
            /* 2. When t is in C, we have everything we need to re-compute the
             * view, as an honest signer would. We simulate the MPC with one
             * fewer party; the unopned party's values are all set to zero. */
            size_t unopened = sig->challengeP[indexOf(
              sig->challengeC, params->numOpenedRounds, t)];
            size_t tapeLengthBytes = getTapeSizeBytes(params);
            if (unopened != last) {// sig->proofs[t].aux is only set when P_t !=
                                   // N
                setAuxBits(&tapes[t], sig->proofs[t].aux, params);
            }
            memset(tapes[t].tape[unopened], 0, tapeLengthBytes);
            memcpy(msgs[t].msgs[unopened],
              sig->proofs[t].msgs,
              params->andSizeBytes);
            msgs[t].unopened = unopened;

            int rv = simulateOnline((uint32_t *)sig->proofs[t].input,
              &tapes[t],
              tmp_shares,
              &msgs[t],
              plaintext,
            ...
            if (rv != 0) {
                PRINT_DEBUG(
                  ("MPC simulation failed for round %lu, signature invalid\n",
                    t));
    ...
    ret = addMerkleNodes(
      treeCv, missingLeaves, missingLeavesSize, sig->cvInfo, sig->cvInfoLen);
    free(missingLeaves);
    if (ret != 0) {
        ret = -1;
        goto Exit;
    ...
    if (memcmp(sig->challengeHash, challengeHash, params->digestSizeBytes)
        != 0) {
...
int sign_picnic3(uint32_t *privateKey,
  uint32_t *pubKey,
  uint32_t *plaintext,
  const uint8_t *message,
  size_t messageByteLength,
  ...
  paramset_t *params)
{
    ...
    commitments_t Cv = { 0 };
    uint8_t *saltAndRoot =
      malloc(params->saltSizeBytes + params->seedSizeBytes);

    computeSaltAndRootSeed(saltAndRoot,
      params->saltSizeBytes + params->seedSizeBytes,
      privateKey,
      pubKey,
      plaintext,
      message,
    ...
    tree_t *iSeedsTree = generateSeeds(params->numMPCRounds,
      saltAndRoot + params->saltSizeBytes,
      sig->salt,
      0,
      params);
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        seeds[t] =
          generateSeeds(params->numMPCParties, iSeeds[t], sig->salt, t, params);
        createRandomTapes(&tapes[t], getLeaves(seeds[t]), sig->salt, t, params);
    }
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        msgs_t *msgst = &msgs[t];
        printf("The messages for t=%ld have pos=%ld and unopened=%d\n",
          t,
          msgst->pos,
          msgst->unopened);
        for (size_t p = 0; p < params->numMPCParties; p++) {
            printf("p: %ld ", p);
            printHex("msg", msgst->msgs[p], 32);
        }
    }
#endif
    shares_t *tmp_shares = allocateShares(params->stateSizeBits);
    for (size_t t = 0; t < params->numMPCRounds; t++) {
#ifdef DEBUG_LMC
        printf("\nOnline phase for MPC simulation number %ld\n", t + 1);
#endif
#ifdef DEBUG_MSG
        printf("\nOnline phase for MPC simulation number %ld\n", t + 1);
#endif
        ...
        int rv = simulateOnline(maskedKey,
          &tapes[t],
          tmp_shares,
          &msgs[t],
          plaintext,
    ...
    allocateCommitments2(&Cv, params, params->numMPCRounds);
#ifdef DEBUG_MSG
    printf("\nDoing a commit_v with msgs->pos=%ld\n", msgs->pos);
#endif
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        commit_h(Ch.hashes[t], &C[t], params);
    ...
    size_t cvInfoLen = 0;
    uint8_t *cvInfo =
      openMerkleTree(treeCv, missingLeaves, missingLeavesSize, &cvInfoLen);
    sig->cvInfo = cvInfo;
    ...
    sig->iSeedInfoLen = revealSeeds(iSeedsTree,
      challengeC,
      params->numOpenedRounds,
      sig->iSeedInfo,
      params->numMPCRounds * params->seedSizeBytes,
      params);
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        if (contains(challengeC, params->numOpenedRounds, t)) {
            ...
            proofs[t].seedInfo =
              malloc(params->numMPCParties * params->seedSizeBytes);
            proofs[t].seedInfoLen = revealSeeds(seeds[t],
              hideList,
              1,
              proofs[t].seedInfo,
              params->numMPCParties * params->seedSizeBytes,
              params);
            proofs[t].seedInfo =
              realloc(proofs[t].seedInfo, proofs[t].seedInfoLen);
            ...
            memcpy(proofs[t].msgs,
              msgs[t].msgs[challengeP[P_index]],
              params->andSizeBytes);
            memcpy(proofs[t].C,
    ...
    int rv = verify_picnic3(
      sig, pubKey, plaintext, message, messageByteLength, params);
    if (rv != 0) {
...
int deserializeSignature2(signature2_t *sig,
  const uint8_t *sigBytes,
  size_t sigBytesLen,
  paramset_t *params)
{
    ...
    if (sigBytesLen < bytesRequired) { return EXIT_FAILURE; }
...
printHex("salt", sigBytes + params->digestSizeBytes, params->saltSizeBytes);
    ...
    expandChallengeHash(
      sig->challengeHash, sig->challengeC, sig->challengeP, params);

    /* Add size of iSeeds tree data */
    sig->iSeedInfoLen = revealSeedsSize(
      params->numMPCRounds, sig->challengeC, params->numOpenedRounds, params);
    bytesRequired += sig->iSeedInfoLen;

    /* Add the size of the Cv Merkle tree data */
    size_t missingLeavesSize = params->numMPCRounds - params->numOpenedRounds;
    uint16_t *missingLeaves = getMissingLeavesList(sig->challengeC, params);
    sig->cvInfoLen = openMerkleTreeSize(
      params->numMPCRounds, missingLeaves, missingLeavesSize, params);
    ...
    size_t seedInfoLen =
      revealSeedsSize(params->numMPCParties, hideList, 1, params);
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        if (contains(sig->challengeC, params->numOpenedRounds, t)) {
            size_t P_t = sig->challengeP[indexOf(
              sig->challengeC, params->numOpenedRounds, t)];
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        if (contains(sig->challengeC, params->numOpenedRounds, t)) {
            ...
            if (P_t != (params->numMPCParties - 1)) {
                ...
                if (!arePaddingBitsZero(sig->proofs[t].aux,
                      3 * params->numRounds * params->numSboxes)) {
...
#ifdef DEBUG_SER
            printf("round %ld: ", t);
            printHex("input", sig->proofs[t].input, params->stateSizeBytes);
            printf("round %ld: ", t);
            printHex("aux", sig->proofs[t].aux, 2 * params->andSizeBytes);
            printf("round %ld: ", t);
            printHex("msgs", sig->proofs[t].msgs, 2 * params->andSizeBytes);
#endif
        }
    }
...
int serializeSignature2(const signature2_t *sig,
  uint8_t *sigBytes,
  size_t sigBytesLen,
  paramset_t *params)
{
    ...
    size_t bytesRequired =
      params->digestSizeBytes + params->saltSizeBytes; /* challenge and salt */

    bytesRequired += sig->iSeedInfoLen; /* Encode only iSeedInfo, the length
    ...
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        if (contains(sig->challengeC, params->numOpenedRounds, t)) {
            memcpy(
              sigBytes, sig->proofs[t].seedInfo, sig->proofs[t].seedInfoLen);
            sigBytes += sig->proofs[t].seedInfoLen;
