...
#define Motorist_Wrap_LastCryptAndMeta \
    (Motorist_Wrap_LastCryptData | Motorist_Wrap_LastMetaData)
...
#define KCP_DeclarePistonsStructure(prefix, size, alignment)            \
    ...
    {                                                                   \
        ...
        unsigned char                                                   \
          indexCrypt; /* indexes only used by parallelized instances */ \
...
#define KCP_DeclareEngineStructure(prefix, size, alignment)          \
    ...
    {                                                                \
        ...
        unsigned char                                                \
          tagEndIndexNext; /* only used by parallelized instances */ \
...
#define KCP_DeclareMotoristFunctions(prefix)                                \
    ...
    int prefix##_Motorist_StartEngine(prefix##_Motorist_Instance *instance, \
      const unsigned char *SUV, size_t SUVlen, int tagFlag,                 \
      unsigned char *tag, int unwrapFlag, int forgetFlag);                  \
    int prefix##_Motorist_Wrap(prefix##_Motorist_Instance *instance,        \
      const unsigned char *input, size_t dataSizeInBytes,                   \
      unsigned char *output, const unsigned char *AD, size_t ADlen,         \
      unsigned char *tag, int unwrapFlag, int forgetFlag, int lastFlags,    \
      size_t *processedIlen, size_t *processedAlen);
...
#define KCP_DeclareKeyakFunctions(prefix)                                     \
    int prefix##Keyak_Initialize(prefix##Keyak_Instance *instance,            \
      const unsigned char *key, unsigned int keySizeInBytes,                  \
      const unsigned char *nonce, unsigned int nonceSizeInBytes, int tagFlag, \
      unsigned char *tag, int unwrapFlag, int forgetFlag);                    \
    int prefix##Keyak_Wrap(prefix##Keyak_Instance *instance,                  \
      const unsigned char *input, unsigned char *output,                      \
      size_t dataSizeInBytes, const unsigned char *AD, size_t ADlen,          \
      unsigned char *tag, int unwrapFlag, int forgetFlag);                    \
    int prefix##Keyak_WrapPartial(prefix##Keyak_Instance *instance,           \
      const unsigned char *input, unsigned char *output,                      \
      size_t dataSizeInBytes, const unsigned char *AD, size_t ADlen,          \
      unsigned char *tag, int unwrapFlag, int forgetFlag, int lastFlags,      \
      size_t *processedIlen, size_t *processedAlen);
