...
#define KRAVATTE_FLAG_LAST_PART \
    2 /* If set, indicates the last part of input/output */
#define KRAVATTE_FLAG_SHORT \
    4 /* If set, indicates Short-Kravatte will be performed */
 ...
 */
int Kravatte_MaskDerivation(
  Kravatte_Instance *kvInstance, const BitSequence *Key, BitLength KeyBitLen);
 ...
 * @param  inputBitLen     The number of bits provided in the input message
 * data. This must be a multiple of 8 if KRAVATTE_FLAG_LAST_PART flag not set.
 * @param  flags           Bitwise or combination of KRAVATTE_FLAG_NONE,
 * KRAVATTE_FLAG_INIT, KRAVATTE_FLAG_LAST_PART.
 * @return 0 if successful, 1 otherwise.
 */
int Kra(Kravatte_Instance *kvInstance, const BitSequence *input,
  BitLength inputBitLen, int flags);

/**
 * Function to expand output data.
 * @param  kvInstance      Pointer to the hash instance initialized by
 * Kravatte_MaskDerivation().
 * @param  output          Pointer to the buffer where to store the output data.
 * @param  outputBitLen    The number of output bits desired.
 *                         This must be a multiple of 8 if
 * KRAVATTE_FLAG_LAST_PART flag not set.
 * @param  flags           Bitwise or combination of KRAVATTE_FLAG_NONE,
 * KRAVATTE_FLAG_SHORT, KRAVATTE_FLAG_LAST_PART.
 * @return 0 if successful, 1 otherwise.
 */
int Vatte(Kravatte_Instance *kvInstance, BitSequence *output,
  BitLength outputBitLen, int flags);
 ...
 * @param  outputBitLen    The number of output bits desired.
 * @param  flags           Bitwise or combination of KRAVATTE_FLAG_NONE,
 * KRAVATTE_FLAG_INIT, KRAVATTE_FLAG_SHORT, KRAVATTE_FLAG_LAST_PART.
 *                         KRAVATTE_FLAG_LAST_PART is internally forced to true
 * for input and output.
 * @return 0 if successful, 1 otherwise.
 */
int Kravatte(Kravatte_Instance *kvInstance, const BitSequence *input,
