...
typedef struct tree_t
{
    ...
    uint8_t *haveNode; /* If we have the data (seed or hash) for node i,
                          haveSeed[i] is 1 */
    uint8_t *exists; /* Since the tree is not always complete, nodes marked 0
                        don't exist */
 ...
 * callers to allocate is numLeaves*params->seedSizeBytes, or call
 * revealSeedsSize. */
tree_t *generateSeeds(size_t nSeeds, uint8_t *rootSeed, uint8_t *salt,
  size_t repIndex, paramset_t *params);
size_t revealSeeds(tree_t *tree, uint16_t *hideList, size_t hideListSize,
  uint8_t *output, size_t outputLen, paramset_t *params);
size_t revealSeedsSize(
  size_t numNodes, uint16_t *hideList, size_t hideListSize, paramset_t *params);
int reconstructSeeds(tree_t *tree, uint16_t *hideList, size_t hideListSize,
  uint8_t *input, size_t inputLen, uint8_t *salt, size_t repIndex,
  paramset_t *params);
 ...
 *     3. openMerkleTree   with missingLeaves - list of commitments the verifier
 * won't recompute
 *     4. freeTree
 *  Verifier call sequence
 *      1. createTree
 *      2. addMerkleNodes       with the output of the signer
 *      3. verifyMerkleTree     Checks that all leaf nodes present are correct
 * commitments
 *      4. freeTree
 */
void buildMerkleTree(
  tree_t *tree, uint8_t **leafData, uint8_t *salt, paramset_t *params);
uint8_t *openMerkleTree(tree_t *tree, uint16_t *missingLeaves,
  size_t missingLeavesSize, size_t *outputSizeBytes);
size_t openMerkleTreeSize(size_t numNodes, uint16_t *notMissingLeaves,
  size_t notMissingLeavesSize, paramset_t *params);
int addMerkleNodes(tree_t *tree, uint16_t *missingLeaves,
  size_t missingLeavesSize, uint8_t *input, size_t inputSize);
