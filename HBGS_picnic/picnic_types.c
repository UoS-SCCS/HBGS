...
void allocateSignature2(signature2_t *sig, paramset_t *params)
{
    ...
    sig->cvInfoLen = 0;
    sig->challengeC =
      (uint16_t *)malloc(params->numOpenedRounds * sizeof(uint16_t));
    sig->challengeP =
      (uint16_t *)malloc(params->numOpenedRounds * sizeof(uint16_t));
...
seeds_t *allocateSeeds(paramset_t *params)
{
    ...
    uint8_t *slab1 =
      malloc((params->numMPCRounds * nSeeds) * params->seedSizeBytes
             + params->saltSizeBytes);// Seeds
    uint8_t *slab2 = malloc(
      params->numMPCRounds * nSeeds * sizeof(uint8_t *) + sizeof(uint8_t *)
      + params->numMPCRounds * sizeof(uint8_t *));// pointers to seeds
    uint8_t *slab3 =
      malloc((params->numMPCRounds) * params->seedSizeBytes
             + params->saltSizeBytes);// iSeeds, used to derive seeds

    // We need multiple slabs here, because the seeds are generated with one
    // call to the KDF; they must be stored contiguously
    ...
    if (params->numMPCParties == 3) {
        seeds[params->numMPCRounds].iSeed =
          slab1;// For ZKB parameter sets, the salt must be derived with the
                // seeds
    } else {
        seeds[params->numMPCRounds].iSeed =
          slab3;// For Pincic2 paramter sets, the salt is dervied with the
                // initial seeds
    }
...
commitments_t *allocateCommitments(paramset_t *params, size_t numCommitments)
{
    commitments_t *commitments =
      malloc(params->numMPCRounds * sizeof(commitments_t));

    commitments->nCommitments =
      (numCommitments) ? numCommitments : params->numMPCParties;

    uint8_t *slab =
      malloc(params->numMPCRounds
             * (commitments->nCommitments * params->digestSizeBytes
...
void allocateCommitments2(
  commitments_t *commitments, paramset_t *params, size_t numCommitments)
{
    commitments->nCommitments = numCommitments;

    uint8_t *slab = malloc(numCommitments * params->digestSizeBytes
                           + numCommitments * sizeof(uint8_t *));
...
{
    if (commitments != NULL) {
        if (commitments->hashes != NULL) { free(commitments->hashes); }
    }
}

inputs_t allocateInputs(paramset_t *params)
{
    uint8_t *slab = calloc(1,
      params->numMPCRounds
        * (params->stateSizeWords * sizeof(uint32_t) + sizeof(uint8_t *)));
...
void freeInputs(inputs_t inputs) { free(inputs); }
...
view_t **allocateViews(paramset_t *params)
{
    ...
    for (size_t i = 0; i < params->numMPCRounds; i++) {
        ...
        for (size_t j = 0; j < 3; j++) { allocateView(&views[i][j], params); }
    }
...
{
    for (size_t i = 0; i < params->numMPCRounds; i++) {
        for (size_t j = 0; j < 3; j++) { freeView(&views[i][j]); }
        free(views[i]);
    }
...
g_commitments_t *allocateGCommitments(paramset_t *params)
{
    ...
    if (params->transform == TRANSFORM_UR) {
        ...
        uint8_t *slab =
          malloc(params->UnruhGWithInputBytes * params->numMPCRounds * 3);
        for (uint32_t i = 0; i < params->numMPCRounds; i++) {
