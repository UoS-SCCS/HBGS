...
{
    uint8_t *seedInfo;// Information required to compute the tree with seeds of
                      // of all opened parties
...
typedef struct signature2_t
{
    uint8_t *salt;
    uint8_t
      *iSeedInfo;// Info required to recompute the tree of all initial seeds
    size_t iSeedInfoLen;
    uint8_t *cvInfo;// Info required to check commitments to views (reconstruct
                    // Merkle tree)
    size_t cvInfoLen;
 ...
 ******************************************************************************/
int sign_picnic_test5(uint32_t *privateKey,
  uint32_t *pubKey,
  uint32_t *plaintext,
  uint32_t *pubKey2,
  uint32_t *plaintext2,
...
int verify_picnic3_test5(signature2_t *sig,
  const uint32_t *pubKey,
  const uint32_t *plaintext,
  const uint32_t *pubKey2,
  const uint32_t *plaintext2,
  const uint8_t *message,
  size_t messageByteLength,
  paramset_t *params);

int serializeSignature2_test5(const signature2_t *sig,
  uint8_t *sigBytes,
  size_t sigBytesLen,
  paramset_t *params);

int deserializeSignature2_test5(signature2_t *sig,
  const uint8_t *sigBytes,
  size_t sigBytesLen,
  paramset_t *params);
