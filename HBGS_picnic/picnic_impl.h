...
#ifdef DEBUG
#define PRINT_DEBUG(printf_args)                         \
...
typedef struct proof_t
{
    uint8_t *seed1;
    uint8_t *seed2;
    uint32_t *inputShare;// Input share of the party which does not derive it
                         // from the seed (not included if challenge is 0)
    uint8_t *communicatedBits;
    uint8_t *view3Commitment;
    uint8_t *view3UnruhG;// we include the max length, but we will only
                         // serialize the bytes we use
} proof_t;
...
void matrix_mul(uint32_t *output, const uint32_t *state, const uint32_t *matrix,
